...
export function Sidebar() {
  ...
  return (
    <aside className="w-full md:w-72 shrink-0 md:sticky md:top-20 md:self-start z-30">
      <div className="bg-white rounded-3xl border border-slate-100/80 shadow-[0_4px_24px_rgba(0,43,127,0.03)] p-5 sm:p-6 flex flex-col justify-between h-[calc(100vh-7rem)] min-h-[calc(100vh-7rem)] overflow-y-auto">
          ...
          {isAdminRoute ? (
            <>
              <div className="px-2 pt-1 flex items-center justify-between">
                <p className="text-xs sm:text-sm font-black uppercase tracking-wider text-[#002B7F]">
                  Espace Administration
                </p>
                <span className="w-3 h-3 rounded-full bg-[#002B7F]" title="Espace Administrateur" />
              </div>
                ...
                {adminNavItems.map((item) => {
                  if (item.disabled || !item.href) {
                    return (
                      <div
                        key={item.label}
                        className="flex flex-col px-4 py-3.5 rounded-2xl text-sm bg-[#F4F7FB] text-[#071530] select-none cursor-default"
                      >
                        <span className="truncate leading-tight font-black text-sm text-[#071530]">
                          {item.label}
                        </span>
                        <span className="text-xs text-[#475569] font-medium truncate mt-0.5">
                          {item.description}
                        </span>
                      </div>
                    );
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        'flex flex-col px-4 py-3.5 rounded-2xl text-sm transition-all duration-200',
                        isActive
                          ? 'bg-[#E2ECFC] text-[#002B7F] font-black shadow-xs'
                          : 'bg-[#F4F7FB] text-[#1E293B] hover:bg-[#EBF2FC] hover:text-[#002B7F]'
                      )}
                    >
                      <span className="truncate leading-tight font-black text-sm">{item.label}</span>
                      <span className="text-xs text-[#475569] font-medium truncate mt-0.5">
                        {item.description}
                      </span>
                    </Link>
            ...
            <>
              <div className="px-2 pt-1 flex items-center justify-between">
                ...
                <span className="w-3 h-3 rounded-full bg-[#002B7F]" title="Espace Technicien" />
              </div>
            ...
            <>
              <div className="px-2 pt-1 flex items-center justify-between">
                ...
                <span className="w-3 h-3 rounded-full bg-[#002B7F]" title="Espace Demandes" />
              </div>
              ...
              <div className="space-y-2.5">
                {demandeNavItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        'flex items-center px-4 py-3.5 rounded-2xl text-sm transition-all duration-200',
                        isActive
                          ? 'bg-[#E2ECFC] text-[#002B7F] font-black shadow-xs'
                          : 'bg-[#F4F7FB] text-[#1E293B] hover:bg-[#EBF2FC] hover:text-[#002B7F]'
                      )}
                    >
                      <span className="truncate font-black text-sm">{item.label}</span>
                    </Link>
                  );
                })}
                ...
                <div className="pt-2 border-t border-slate-100">
                  ...
                  >
                    <span className="truncate font-black text-sm">Fiche d&apos;Assistance</span>
                  </Link>
                </div>
              </div>
            </>
        ...
        <div className="pt-6 mt-6 border-t border-slate-100">
          <Link
            href="/profil"
            className={cn(
              'p-4 rounded-2xl transition-all duration-200 flex items-center justify-between group',
              isProfilActive
                ? 'bg-[#E2ECFC]'
          ...
          >
            <div className="flex items-center gap-3.5 overflow-hidden">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-white font-black text-sm shrink-0 bg-[#002B7F] shadow-xs">
            ...
            <span className="text-sm font-black text-[#002B7F] group-hover:translate-x-0.5 transition-all shrink-0">&rarr;</span>
          </Link>
        </div>
